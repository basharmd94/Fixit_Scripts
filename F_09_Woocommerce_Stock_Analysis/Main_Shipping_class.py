...
df_compare_prev_day_stock=df_compare_prev_day_stock.drop(columns={'Product Name Central','Central Inventory','fixit_Inventory','ecom_erp_Inventory'}).rename(columns={'sum_of_stock':'prev_day_stock'})
...
wcapi = API(
  ...
  #ck_a9a6e717ee66b754abfbeb4fe7adcd01ea1aff19 old key
  #cs_dbce9028902815c56bfe8625d0fe7e717172eaae old secret
...
########## try except  here.  in except if get error read df_woocommerce_excel file. ###############
f = open("log.txt", "a")
...
df_woocommerce_data = pd.DataFrame (flat_3, columns = ['id','sku','shipping_class'])
# df_woocommerce_data_for_not_seo = pd.DataFrame (flat_3, columns = ['id','sku','shipping_class','meta_data']).to_excel("seoNotComplete.xlsx")
df_woocommerce_data_for_categories = pd.DataFrame (flat_3, columns = ['id','sku','shipping_class','categories'])
df_woocommerce_data_for_categories.to_excel("categories.xlsx" , index= False)
...
# if there is no data in df_shipping_class_need_to_change programs stopped at try point and sent a mail that no data to update
# %%
...
############## create json data for posting to api    
    try:
        ...
        #raise SystemExit(e)
f.close()
df_merging_all_stock.to_excel('prev_day_stock.xlsx' , index = False)
# %%
