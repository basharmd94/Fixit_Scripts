"""
🚀 F_02_02_Gulshan.py – Item Bulk Edit and Upload for Gulshan Warehouse

📌 PURPOSE:
    - Bulk upload and edit items for Gulshan warehouse (zid: 100001)
...
🔧 DATA SOURCES:
    - Input: new_products.xlsx (output from F_02_01_Central.py)
...
🏢 INPUT:
    - ZID_GULSHAN: 100001 (Gulshan warehouse)
    - Input file: new_products.xlsx

📧 EMAIL:
    - Recipients: get_email_recipients("F_02_02_Gulshan")
...
# === 3. Configuration ===
ZID_GULSHAN = 100001  # Gulshan warehouse ID
INPUT_FILE = 'new_products.xlsx' # This file is an output from F_02_01_Central.py
OUTPUT_FILE = 'F_02_02_Gulshan_Products.xlsx'
TODAY_DATE = date.today().strftime("%Y-%m-%d")

print(f"📌 Processing for: Gulshan Warehouse (ZID={ZID_GULSHAN})")
...
df_all_item_from_erp = get_all_item_from_erp(ZID_GULSHAN)
...
def get_all_xitem_from_erp_by_pattern(itemcode):
    ...
    df_items_code = pd.read_sql(
        sql_statement, 
        con=engine, 
        params={'zid': ZID_GULSHAN, 'pattern': f"{itemcode}%"}
    )['xitem'].to_list()
...
for key, value in duplicate_item_code.items():
    df_new_list = get_all_xitem_from_erp_by_pattern(key)
    if df_new_list:
...
df_not_available_in_erp['xwh'] = 'Fixit Gulshan'
...
subject = f"Fixit-02 Gulshan Item Bulk Upload – {TODAY_DATE}"
body_text = "Please find attached the processed item data for Gulshan warehouse."

send_mail(
    subject=subject,
    bodyText=body_text,
    html_body=[(df_all_rows.reset_index(), "Gulshan Item Upload Results")],
